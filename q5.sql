...
WITH gdp AS (
	SELECT
		year_value,
		gdp
	FROM t_vit_sterba_project_sql_secondary_final	
...
prev_gdp AS (
	SELECT
		year_value,
		gdp,
		LAG(gdp) OVER (ORDER BY year_value) AS previous_gdp
	FROM gdp
),
gdp_rise AS (
	SELECT
...
SELECT
	pa.year_value,
	ROUND(g.gdp_rise::numeric,2) AS gdp_rise,
	ROUND(avg_price_rise::numeric,2) AS price_rise,
	ROUND(avg_payroll_rise::numeric,2) AS payroll_rise
FROM avg_payroll_rise pa
JOIN avg_price_rise pr ON pa.year_value = pr.year_value
JOIN gdp_rise g ON pa.year_value = g.year_value
WHERE g.gdp_rise IS NOT NULL AND pa.avg_payroll_rise IS NOT NULL AND pr.avg_price_rise IS NOT NULL
ORDER BY g.year_value;